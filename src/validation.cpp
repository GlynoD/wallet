...
#if defined(NDEBUG)
# error "GLYNO Core cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "GLYNO Signed Message:\n";
  ...
  int halvings = nHeight / consensusParams.nSubsidyHalvingInterval;
    ...
    if (halvings >= 30)
        return 0;
          ...
          if (nHeight == 1)
              return 378432 * COIN;  // GLYNO PREMINE 3 % of 12614400
          if (nHeight >= 2 && nHeight <= 30)
              return 2 * COIN;  // Instamine Protection 1 GLYNO per block
          if (nHeight >= 31 && nHeight <= 90)
              return 4 * COIN; // Instamine Protection 4 GLYNO per block
          if (nHeight >= 91 && nHeight <= 150)
              return 6 * COIN; // Instamine Protection 6 GLYNO per block
          if (nHeight >= 151 && nHeight <= 210)
              return 8 * COIN; // Instamine Protection 8 GLYNO per block
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // GLYNO : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(GLYNO): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(GLYNO): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // GLYNO : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(GLYNO): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(GLYNO): spork is off, skipping transaction locking checks\n");
    }
