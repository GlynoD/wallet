...
*Setup instructions for a Gitian build of GLYNO Core using a Debian VM or physical system.*
...
- [Building GLYNO Core](#building-glyno-core)
...
Clone the git repositories for GLYNO Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/MasternodeFoundation/glyno
```
...
in the GLYNO Core repository under 'Fetch and build inputs' to install sources which require
...
To build GLYNO Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the GLYNO Core repository.
...
```bash
    ...
    From https://github.com/MasternodeFoundation/glyno
...
For example:
```bash
URL=https://github.com/crowning-/glyno.git
...
and the glyno git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../glyno/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/glyno.git
SIGPATH=/some/root/path/glyno-detached-sigs.git

./bin/gbuild --url glyno=${BTCPATH},signature=${SIGPATH} ../glyno/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/glyno-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/glyno-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/glyno-osx-build.assert
```
...
[glyno/gitian.sigs](https://github.com/MasternodeFoundation/gitian.sigs/) repository, or if that's not possible to create a pull
request.
